...
export async function uploadContractAction(
  ...
  // Внешний предохранитель: что бы ни пошло не так ниже (незнакомая структура
  // PDF, временный сбой Blob/БД и т.п.) — родитель должен увидеть понятную
  // ошибку с возможностью повторить, а не падение страницы целиком.
  try {
